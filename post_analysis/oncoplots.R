...
theta <- as.matrix(read.table("../classification_results/thetas_fastMHN/theta_group11.dat", header = FALSE, sep = " ", stringsAsFactors = FALSE, check.names = FALSE))
colnames(theta) <- genes; rownames(theta) <- c(genes, "Observation")

# load final group memberships
mships <- readLines("../classification_results/classification_fastMHN_13groups.dat")
